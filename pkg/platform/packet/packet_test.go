...
func Test_areUniqueStrings(t *testing.T) {
	tests := []struct {
		name    string
		strs    []string
		want    []string
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			got, err := areUniqueStrings(tt.strs)
			if (err != nil) != tt.wantErr {
				t.Errorf("areUniqueStrings() error = %v, wantErr %v", err, tt.wantErr)

				return
			}

			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("areUniqueStrings() = %v, want %v", got, tt.want)
			}
		})
	}
}
